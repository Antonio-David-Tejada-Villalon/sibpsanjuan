...
const SUBTIPOS = [
  ...
  { valor: "postal", etiqueta: "Postal" },
  { valor: "lamina", etiqueta: "Lámina" },
  { valor: "afiche", etiqueta: "Afiche" },
...
function mapEntidadAForm(item) {
  return {
    ...
    anio: item.anio || "",
    subtipo: item.subtipo || "fotografia",
...
export default function MaterialGrafico() {
  const {
    ...
    listar: (pagina, porPagina, q) => api.listarMaterialGraficoPaginado(pagina, porPagina, q),
    crear: (datos, form) => api.crearMaterialGrafico({ ...datos, ejemplares: aEjemplares(form.ejemplaresTexto) }),
    actualizar: (id, datos) => api.actualizarMaterialGrafico(id, datos),
  ...
  return (
    <>
      ...
      <form className="card" onSubmit={onSubmit}>
        <h2>{editandoId ? "Editar material gráfico" : "Nuevo material gráfico"}</h2>
        ...
        <Campo
          nombre="autoresTexto"
          etiqueta="Fotógrafo/a o artista (separados por punto y coma) (100$a / 700$a)"
          form={form}
          onCambiar={onCambiarCampo}
        />
        <p>
        ...
        <Campo
          ...
          onCambiar={onCambiarCampo}
        />
        <Campo nombre="tecnica" etiqueta="Técnica (ej: blanco y negro) (340$a)" form={form} onCambiar={onCambiarCampo} />
        <Campo
        ...
        {editandoId ? (
          ...
          <label>
            ...
            <textarea
              rows={3}
              placeholder={"BPGRAF-000001, 770 FOT\nBPGRAF-000002, 770 FOT"}
          ...
          {seleccionados.size > 0 && (
            <div className="barra-seleccion">
              ...
              <button
                type="button"
                className="peligro"
                onClick={() => onEliminarSeleccionados("¿Eliminar los materiales gráficos seleccionados?")}
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item._id} className={eliminandoId === item._id ? "saliendo" : ""}>
                  ...
                  <td>
                    ...
                    <button className="peligro" onClick={() => onEliminar(item._id, "¿Eliminar este material gráfico?")}>
                      Eliminar
                    </button>
                  </td>
                </tr>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={5}>
                    {busqueda
                      ? "Ningún material gráfico coincide con la búsqueda."
                      : "Todavía no cargaste ningún material gráfico."}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Pager pagina={pagina} porPagina={POR_PAGINA} total={total} onCambiar={setPagina} etiqueta="material gráfico" />
    </>
  );
}
